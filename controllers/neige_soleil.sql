...
BEGIN
    IF (SELECT mdp_user FROM user u WHERE u.email_user = email) = mdp THEN
        UPDATE user u
            SET
                u.last_connexion = NOW(),
                u.nb_connexion = u.nb_connexion+1,
                u.etat_connexion = "Reussi"
            WHERE u.email_user = email; 
    ELSE
        UPDATE user u
            SET
                u.last_connexion = NOW(),
                u.last_failConnexion = NOW(),
                u.nb_connexion = u.nb_connexion+1,
                u.nb_failConnexion = u.nb_failConnexion+1,
                u.etat_connexion = "Echec"
            WHERE u.email_user = email;
    END IF;
END //
DELIMITER ;
...
DROP TRIGGER IF EXISTS insert_verifUser;
DELIMITER //
CREATE TRIGGER insert_verifUser
...
BEGIN
    ...
    IF nb_ligne = 0 THEN
        INSERT INTO verif_connexion (nom, prenom, email, date_connexion,etat, id_user) values (NEW.nom_user, NEW.prenom_user, NEW.email_user, new.last_connexion,new.etat_connexion, new.id_user);
    ELSE    
    SELECT last_connexion into date_r FROM user  WHERE id_user = new.id_user;   
